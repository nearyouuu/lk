...
Документ описывает frontend для электронного журнала и ведомости контрольных точек. Журнал работает независимо от расписания: группы, студенты и дисциплины загружаются автоматически, а преподаватель вручную создаёт колонки «дата + часы», отмечает посещаемость и выставляет оценки. Тема остаётся необязательной. Актуальные изменения по часовой модели собраны в `FRONTEND_JOURNAL_HOURS_CHANGES.md`.
...
| `journal.lesson.write` | Создание и изменение колонок журнала |
...
Преподаватель видит только назначенные ему пары «группа — дисциплина — учебный год — семестр». `journal_assignments` является единственным источником преподавательского доступа: наличие занятия в расписании само по себе доступ к журналу не выдаёт. Администратор и директор видят все данные.
...
2. Вкладка «Учебный журнал»: студенты, даты, часы, посещаемость и оценки.
...
Группы и дисциплины берутся из этого ответа. Каталог строится только по активным `journal_assignments` и не объединяется с расписанием. Не нужно заставлять преподавателя вручную создавать группы и дисциплины в журнале.
...
`schedule_lesson_id` может быть `null`: занятие не обязано быть связано с расписанием. Если поле заполнено, backend использует его только как ссылку на источник и проверяет совпадение группы и дисциплины. Одна запись расписания может быть добавлена в журнал только один раз.

В ответе занятия присутствует поле:
...
`source` принимает `manual` или `schedule`. Frontend должен отображать в основной таблице только `journal_lessons`; записи расписания нельзя самостоятельно подмешивать в массив занятий журнала.

Если занятие создаёт преподаватель, `teacher_id` определяется по текущему пользователю. Администратор может указать `teacher_id`; выбранный преподаватель должен иметь активное назначение на этот журнал. Если назначен один преподаватель, backend выберет его автоматически. Если назначено несколько, администратор должен явно выбрать проводящего занятие.
...
Эта настройка является единственным источником доступа преподавателя к журналу и позволяет работать без связи с расписанием.
...
Для администратора рекомендуется отдельная таблица назначений с фильтрами по преподавателю, группе, дисциплине и периоду. Назначение открывает доступ сразу ко всему общему журналу: занятиям, посещаемости, оценкам, КТ и рейтинговой ведомости.

Если у одной группы по одной дисциплине назначено несколько преподавателей, все они видят один и тот же журнал и могут редактировать его занятия. `teacher_id` занятия показывает проводящего преподавателя, но не является условием доступа.
...
КТ принадлежит журналу по комбинации `group_id + subject_id + period_id` и не содержит `teacher_id`. Frontend не должен передавать или ожидать преподавателя в запросах и ответах КТ. Доступ проверяется через активное назначение на журнал.
